...
module.exports = {
    start: function(handlerInput, errorDetail) {
        ...
        // Synchronously publish to SNS and Slack (if configured)
        module.exports.publishToSNS(errorMessage);
        if (config.errorNotification.slackWebhook) {
            module.exports.publishToSlack(errorMessage);
        }
    },
    ...
    publishToSNS: function(errorMessage) {
        ...
        errorMessage = removeEmailMarkdown(errorMessage);
    ...
    publishToSlack: async function(errorMessage) {
        
        // Email message will be plaintext, so remove Markdown
        errorMessage = fixSlackMarkdown(errorMessage);

        // Initiate Slack webhook
        const slackWebhook = new IncomingWebhook(config.errorNotification.slackWebhook);
...
function fixSlackMarkdown(text) {
    // Very simple approach to fixing Markdown for Slack:
    // Replace all percent characters by stars
    return text.replace(
        /\%/g, '*'
    );
}